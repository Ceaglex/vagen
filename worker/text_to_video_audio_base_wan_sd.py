...
def main(args, accelerator):
    ...
    joint_va = JointVADiTModel(
        ...
        full_weights_path = args.full_safetensors_path
    )
    ...
    if args.lora_config.use_lora == True:
        ...
        if hasattr(args.lora_config, 'lora_load_path') and args.lora_config.lora_load_path:
            ...
            joint_va = PeftModel.from_pretrained(
                joint_va, 
                args.lora_config.lora_load_path,
                adapter_name="learner"
            )
            joint_va = get_peft_model(joint_va, lora_config, adapter_name="ref")  # type: ignore[arg-type]
        else:
            torch.manual_seed(args.seed); random.seed(args.seed)
            joint_va = get_peft_model(joint_va, lora_config, adapter_name="ref")
            torch.manual_seed(args.seed); random.seed(args.seed)
