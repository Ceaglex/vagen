...
def prepare_config():
    ...
    args = OmegaConf.merge(config, cli)
    return args
...
def prepare_everything(args):
    # 设置随机种子
    torch.backends.cudnn.allow_tf32 = args.get("allow_tf32", False)
    torch.set_float32_matmul_precision(args.get("matmul_precision", 'highest'))
    ...
    accelerator = Accelerator(
        ...
        log_with=args.get("report_to", None),
    ...
    os.makedirs(os.path.join(output_dir, args.get("logging_subdir", "logging")), exist_ok=True)
    os.makedirs(os.path.join(output_dir, args.get("wandb_subdir", "wandb")), exist_ok=True)
